...
const Page = (props) => {
  ...
  const content = (page._rawContent || [])
    .filter((c) => !c.disabled)
    .map((c, i) => {
      return <DynamicComponent component={c._type === 'uiComponentRef' ? c.name[0].toUpperCase() + c.name.slice(1) : c._type[0].toUpperCase() + c._type.slice(1)} key={i} />
      // let el = null;
      // switch (c._type) {
      //   case "pricing":
      ...
      //     switch (c.name) {
      //       case "topWave":
      //         el = <TopWave key={c.key} />;
      //         break;
      //       case "bottomWave":
      //         el = <BottomWave key={c.key} />;
      //         break;
      //       default:
      //         break;
      //     }
